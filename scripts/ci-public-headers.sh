#!/usr/bin/env bash
# Mirrors GitHub Actions job: public-headers.
...
if [[ -n "${CMAKE_PRESET:-}" ]]; then
   ...
   echo "==> cmake build public headers ($BUILD_PRESET)"
   cmake --build --preset "$BUILD_PRESET" --target PublicHeaderSelfCheck
else
   ...
   echo "==> cmake build public headers"
   cmake --build "$BUILD_DIR" --config "$BUILD_CONFIG" --target PublicHeaderSelfCheck
fi
